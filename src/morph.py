...
# by optimising chamfer loss iteratively with random perturbations applied to points
# jitter decays from initial_jitter_scale at iteration 0 to 0 at iteration 100
# total points = num_points**2
def morph_sphere_with_jitter(src_pcd_tensor, num_points, iterations, learning_rate, stops=[],
                              loss_func="chamfer", measure_consistency=True, sphere=True,
                              return_assignment=True, initial_jitter_scale=0.01):
    ...
    for i in tqdm(range(iterations)):
        ...
        # Add jitter to sphere_points before loss calculation
        # Jitter scales down from initial_jitter_scale to 0 at iteration 1000
        jitter_scale = initial_jitter_scale * max(0, (1000 - i) / 1000)
        jittered_sphere_points = sphere_points + torch.randn_like(sphere_points) * jitter_scale

        if loss_func == "chamfer":
            nn = chamferDist(
                src_pcd_tensor, jittered_sphere_points, bidirectional=True, return_nn=True)
            loss = torch.sum(nn[1].dists) + torch.sum(nn[0].dists)
            assignment = [nn[0].idx[:,:,0].detach().cpu().numpy(), nn[1].idx[:,:,0].detach().cpu().numpy()]
        elif loss_func == "emd":
            loss, assignment = calc_emd(jittered_sphere_points, src_pcd_tensor, 0.05, 50)
            assignment = assignment.detach().cpu().numpy()
        elif loss_func == "direct":
            loss = torch.sum(torch.square(jittered_sphere_points - src_pcd_tensor))
            assignment = None
        elif loss_func == "pair":
            loss, assignment = get_pair_loss_clouds_tensor(src_pcd_tensor, jittered_sphere_points, add_pair_loss=True, it=i)
        elif loss_func == "jittery":
            loss = get_jittery_cd_tensor(src_pcd_tensor, jittered_sphere_points, k=1, it=i)
        elif loss_func == "self":
            loss = get_self_cd_tensor(src_pcd_tensor, jittered_sphere_points)
        elif loss_func == "reverse":
            loss, assignment = calc_reverse_weighted_cd_tensor(src_pcd_tensor, jittered_sphere_points, return_assignment=True, k=32)
        elif loss_func == "prob":
            loss, assignment = calc_pairing_probabilty_loss_tensor(src_pcd_tensor, jittered_sphere_points, k=64)
        elif loss_func == "balanced":
            loss, assignment = calc_balanced_chamfer_loss_tensor(src_pcd_tensor, jittered_sphere_points, return_assignment=True, k=32)
        elif loss_func == "single":
            loss, assignment = calc_balanced_single_chamfer_loss_tensor(src_pcd_tensor, jittered_sphere_points, return_assignment=True, k=32)
        elif loss_func == "infocd":
            loss, assignment = calc_cd_like_InfoV2(src_pcd_tensor, jittered_sphere_points, return_assignment=True)
        elif loss_func == "density":
            loss = calc_relative_density_loss_tensor(src_pcd_tensor, jittered_sphere_points, return_assignment=False)
        elif loss_func == "curvature":
            loss = calc_balanced_curvature_loss_tensor(src_pcd_tensor, jittered_sphere_points, return_assignment=False)
        elif loss_func == "cyclic":
            loss, assignment = calc_dcd_correspondence_tensor(src_pcd_tensor, jittered_sphere_points, return_assignment=True)
        else:
            print("unspecified loss")
