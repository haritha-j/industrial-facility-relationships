...
def parse_args():
    ...
    parser.add_argument('--pretrained_encoder', type=str, default=None, help='path to pretrained completion model checkpoint')
...
def load_encoder_from_completion_model(regression_model, checkpoint_path, device, logger):
    """
    Load encoder weights from a pre-trained completion model checkpoint.

    Args:
        regression_model: The regression model to load encoder weights into
        checkpoint_path: Path to the completion model checkpoint
        device: Device to load checkpoint on
        logger: Logger for logging messages
    """
    logger.info(f'Loading pretrained encoder from: {checkpoint_path}')

    # Load the completion model checkpoint
    completion_checkpoint = torch.load(checkpoint_path, map_location=device)

    # Extract the state dict
    if 'model_state_dict' in completion_checkpoint:
        completion_state_dict = completion_checkpoint['model_state_dict']
    else:
        completion_state_dict = completion_checkpoint

    # Filter only encoder weights
    encoder_state_dict = {}
    for key, value in completion_state_dict.items():
        if key.startswith('encoder.'):
            encoder_state_dict[key] = value

    # Load encoder weights into the regression model (strict=False allows missing keys for decoder)
    model_state_dict = regression_model.state_dict()
    model_state_dict.update(encoder_state_dict)
    regression_model.load_state_dict(model_state_dict, strict=False)

    logger.info(f'Successfully loaded {len(encoder_state_dict)} encoder parameters')
    return regression_model


def freeze_encoder(model, logger):
    """
    ...
    logger.info(f'Trainable {trainable_params:,} decoder parameters')
...
#                    RandomNoise(),
    ...
    # Load pretrained encoder from completion model if specified
    if args.pretrained_encoder is not None:
        predictor = load_encoder_from_completion_model(predictor, args.pretrained_encoder, device, logger)
        predictor = freeze_encoder(predictor, logger)
        log_string('Loaded and froze pretrained encoder weights')
        start_epoch = 0
    else:
        # Try to load from existing regression checkpoint
        try:
    ...
    trainable_params = filter(lambda p: p.requires_grad, predictor.parameters())
